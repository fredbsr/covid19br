...
# Função resumo
my_ridge = function(data, xvar,yvar, facet.var=NULL,fonte="Lato") {
  data <- data %>% dplyr::filter(!is.na(!! enquo(yvar)))
    p = data %>%
      ggplot(aes(x={{xvar}},y={{yvar}},fill={{yvar}},color={{yvar}},group={{yvar}})) +
      geom_density_ridges() +
      stat_summaryh(fun.x=mean, geom="vline", aes(xintercept=..x..),color="white") +
      facet_grid(rows=vars({{facet.var}}),scales = "free_y",switch = "y") +
      stat_summary(fun.data = "mean_cl_boot", geom="rect", 
                    aes(ymin=-Inf,ymax=Inf#, 
                        #label=paste0("media: ", ..x..)
                        ),
                    fill="white",
                   color=NA,
                    #size=3.5, 
                    #hjust=0
                    alpha=.5) +
      stat_summaryh(fun.x=mean, geom="label", 
                   aes(y=2, 
                       label=paste0("media: ", round(..x..,1))),
                   color="white",
                   family=fonte,
                   fontface="bold",
                   alpha=.65,
...
<h2 class="center"><amar>Quem brinca com a morte corre o risco de morrer politicamente sozinho.</amar></h2>
